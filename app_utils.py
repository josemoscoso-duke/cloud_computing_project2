...
def parse_xml(input_filename: str):
    ...
    # create a txt file and insert normalized values for x, y, height and width
...
def get_image_resolution(input_filename: str):
    """return the size of the dimensions of an square image"""
    tree = ET.parse(input_filename)
    root = tree.getroot()

    for x in root:
        if x.tag == 'size':
            return x.find("width").text
    

def draw_bounding_box(img_path: str, bndbox_path: str, xml_path: str):
    ...
    size = get_image_resolution(xml_path)
    ...
    with open(bndbox_path, 'r') as myfile:
        ...
        for line in lines:
            ...
            xmin = (x-w/2)
            ymin = (y-h/2)
            rect_patches.append(patches.Rectangle((xmin*float(size),ymin*float(size)),
                       w*float(size),h*float(size),linewidth=1,edgecolor='r',facecolor='none'))
